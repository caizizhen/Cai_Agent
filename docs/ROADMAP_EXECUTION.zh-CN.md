...
| `HM-N03-D01` | `Done` | `HM-02` | API 状态路由扩展（health / ready / liveness） | **`GET /v1/health`**（**`api_health_v1`**）、**`GET /v1/ready`**（**`api_ready_v1`**）；**`/healthz`** / **`/health`** 返回 **`api_liveness_v1`**（仍无 Bearer）；RFC **`HM_02_MINIMAL_SERVER_CONTRACT.zh-CN.md`** 已更新 | `HM-02d-openai` | pytest `test_api_http_server` + `test_api_status_routes` |
...
| `HM-N04-D01` | `Done` | `HM-04` | dashboard preview/apply/audit contract 收口 | `ops_http_server` 交互面收口为 **GET 仅 preview/audit**，**POST /v1/ops/dashboard/interactions** 承载 apply（`mode=apply|preview|audit`）；保留 `ops_dashboard_interactions_v1` + `ops_dashboard_action_audit_v1`，并对 GET apply 返回 `execute_forbidden`，确保写动作路径显式且可审计 | `HM-04c` | pytest `test_ops_http_server` |
...
| `ECC-N03-D04` | `Done` | `ECC-N03` | structured home diff（add/update/skip/conflict） | **`ecc_structured_home_diff_v1`** / **`ecc_structured_home_diff_bundle_v1`**（`build_export_ecc_structured_home_diff_v1` / `build_ecc_structured_home_diff_bundle_v1`）、**`cai-agent ecc home-diff`**；**`doctor_v1.ecc_structured_home_diff`**；**`repair_plan_v1.ecc_structured_home_diff_pending_targets`** / **`ecc_home_diff_preview_commands`**；**`doctor_upgrade_hints_v1`** 增补 **`ecc home-diff`** | `ECC-N03-D03` | pytest `test_export_sync_diff` + `test_doctor_cli` + `test_repair_cli` + smoke |
...
| `ECC-N02-D03` | `Done` | `ECC-N02` | pack import（跨工作区安装预览与 apply） | **`ecc pack-import`**（**`--from-workspace`**、**`--dry-run`** / **`--apply`**）、**`skills_hub_pack_install_v1`**；与 manifest/catalog 校验对齐 | `ECC-N02-D02` | pytest `test_ecc_layout_cli` + smoke |
| `ECC-N02-D04` | `Done` | `ECC-N02` | pack repair 诊断与 doctor/repair 挂钩 | **`ecc pack-repair`**、**`ecc_asset_pack_repair_report_v1`**；**`doctor`** / **`repair`** 提示与 **`doctor_upgrade_hints_v1`** | `ECC-N02-D03` | pytest `test_asset_pack_repair` + `test_doctor_cli` + `test_repair_cli` + smoke |
| `ECC-N02-D05` | `Done` | `ECC-N02` | pack apply 与 ingest sanitizer 合流 | **`ecc pack-import --apply`** 写入前扫描源 workspace 的 **`rules`/`skills`/`agents`/`commands`** 下 **`hooks.json`**：复用 **`hook_runtime`** 危险命令判定（**`list_hook_argv_danger_matches`** / **`hook_argv_matches_ingest_denylist`**），并阻断 **`script_outside_workspace`**；**`ecc_asset_pack_import_plan_v1`** 附带 **`ingest_gate`**（**`ecc_pack_ingest_gate_v1`**）；失败返回 **`ingest_gate_rejected`**。**`ecc pack-repair`** 仍为只读诊断无 `--apply` | `ECC-N02-D04` `ECC-N04-D02` | pytest `test_ecc_pack_ingest_gate.py` + `test_ecc_layout_cli.py` + smoke |
| `ECC-N02-D06` | `Done` | `ECC-N02` | README + doctor/API 暴露 pack ingest 预检 | 根 **`README.md` / `README.zh-CN.md`** 增补 **`ecc pack-import`** 与 **`ingest_gate`** 说明；**`doctor_v1.ecc_pack_ingest_gate`**；**`GET /v1/doctor/summary`** 增加 **`ecc_pack_ingest_gate`**（**`api_ecc_pack_ingest_gate_summary_v1`**）；人类 **`doctor`** 增加 ECC ingest 摘要行；**`doctor_upgrade_hints_v1`** 增补 **`ecc pack-import --json`** | `ECC-N02-D05` | pytest `test_doctor_cli` + `test_api_http_server` |
| `ECC-N02-D07` | `Done` | `ECC-N02` | skills hub install 与 ingest 门禁合流 | **`apply_skills_hub_manifest_selection`** 对 manifest 中待复制 **`hooks.json`** 调用 **`build_ecc_pack_ingest_gate_for_explicit_hooks_v1`**；**`skills_hub_pack_install_v1`** 附带 **`ingest_gate`**；非 dry-run 拒绝 **`ingest_gate_rejected`**；**`ecc_pack_ingest_gate_v1`** 增加 **`ingest_scan_kind`** / **`explicit_scanned_paths`**；README 补充 **skills hub install** 说明 | `ECC-N02-D06` | pytest `test_skills_hub_install_ingest` + `test_ecc_pack_ingest_gate` + `test_api_http_server` |
| `ECC-N02-D08` | `Done` | `ECC-N02` | ingest 门禁 smoke + CLI 回归补强 | `scripts/smoke_new_features.py` 新增 **`skills hub install`** 的安全 dry-run 与危险 hooks 拒绝路径；`test_skills_lint_cli` 新增 CLI 级 `ingest_gate_rejected` + exit code 断言，强化命令面回归 | `ECC-N02-D07` | pytest `test_skills_lint_cli` + smoke |
...
| `CC-N03-D04` | `Done` | `CC-N03` | plugins sync-home safe apply | 新增 **`plugins_sync_home_result_v1`** 与 **`plugins sync-home --apply`**：默认阻断目标目录已存在且内容不同的写入，返回 `conflicts[]`；显式 **`--force`** 时先写 `.backup-*` 再替换，`--no-backup` 可关闭备份 | `CC-N03-D03` | pytest `test_plugin_compat_matrix` + 全量 + smoke |
| `CC-N04` | `Done` | `CC-N04` | session/recap 收口（稳定回放叙事） | 新增 **`session_recap_v1`**（`context.build_session_recap_v1`），`sessions --recap [--json]`，TUI 新增 **`/recap`** 并在会话 strip 文案统一暴露；用于稳定输出最近会话摘要与 replay 命令面 | `CC-N03-D04` | pytest `test_session_recap` + `test_tui_session_strip` + 全量 + smoke |
| `UX-N01-D01` | `Done` | `UX-N01` | 体验层第一阶段（Onboarding 优先） | 新增 **`cai-agent onboarding`**（`onboarding_quickstart_v1`）聚合入口，统一 dry-run 引导链路：`init -> doctor -> models onboarding -> run/ui/sessions-recap`；`init`/`doctor`/README/ONBOARDING 对齐 onboarding-first 语义；TUI 任务看板会话提示补齐 `/recap` 可见性 | `CC-N04` | pytest `test_cli_misc` + `test_doctor_cli` + `test_tui_session_strip` + 全量 + smoke |
| `UX-N01-D02` | `Done` | `UX-N01` | 体验层第二阶段（失败提示与 help 可发现性） | 顶层 **`cai-agent --help`** 增加 onboarding quickstart；`models`/`ecc`/`doctor`/`repair`/`tools` 缺配置失败统一输出 onboarding 下一步（`onboarding -> init --preset starter`）；新增 `_print_config_not_found_hint` 收口文案语义 | `UX-N01-D01` | pytest `test_cli_misc` + 全量 + smoke |
| `UX-N01-D03` | `Done` | `UX-N01` | 体验层第三阶段（sessions/continue 可发现性） | `sessions --help` 与 `continue --help` 增加 quickstart/会话定位提示；`sessions` 文本输出在无会话时给出生成会话建议，在有会话时自动给出 `continue` 示例命令，降低续聊路径门槛 | `UX-N01-D02` | pytest `test_cli_misc` + 全量 + smoke |
| `UX-N01-D04` | `Done` | `UX-N01` | 体验层第四阶段（run/continue 失败提示统一） | `run/continue` 在 `plan_file_error`、`load_session_failed`、`invalid_session` 失败场景统一输出下一步 hints；JSON 失败载荷新增 `hints[]`，文本 stderr 同步 `hint:` 行；新增 `_run_continue_failure_hints` 单点维护失败提示语义 | `UX-N01-D03` | pytest `test_cli_misc` + 全量 + smoke |
