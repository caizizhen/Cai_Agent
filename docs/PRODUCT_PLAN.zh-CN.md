...
| 17 | Hermes backlog **S2-02～S2-05**（freshness / conflict_rate / coverage 指标、nudge-report 与 health 联动） | **未开始** | 见 `HERMES_PARITY_PROGRESS` Sprint 2 表 |
| 18 | **S1-02** `docs/schema/` 各命令 JSON schema 文档 | **未开始** | |
| 19 | **S1-03** 全命令 exit 0/2 语义补齐（含 `schedule stats`、`observe-report` 等） | **部分完成** | 与 PROGRESS「部分完成」一致 |
| 20 | **S4-04** 调度审计 JSONL 事件类型统一（7 种标准事件名） | **未开始** | |
...
|------|----------|------|------|----------------|
| T1 | `pytest cai-agent/tests` | 自动化 | **完成** | 例：rebase 后 **315 passed**（本机） |
