...
- **`scripts/smoke_new_features.py`**：在仓库根校验 **`plugins --json`**（**`plugins_surface_v1`**）、**`doctor --json`**（**`doctor_v1`**，**`--config`** 指向根目录 **`cai-agent.toml`**）；在空临时目录校验 **`insights --json`**、**`board --json`**（**`board_v1`** + 内嵌 **`observe`** **`1.1`**）；在隔离目录写入最小 **`hooks/hooks.json`** 校验 **`hooks list --json`**（**`hooks_catalog_v1`**）；在空临时工作区校验 **`memory health --json`**（S2-01 **根 `schema_version`=`1.0`**）；并校验 **`init --json`**、**二次 `init --json`**、**`schedule … --json`**、**`gateway telegram list --json`**、**`recall --json`**、**`memory … --json`** 等；入口为 **`python -m cai_agent`** + **`PYTHONPATH=cai-agent/src`**（与 **`run_regression.py`** 一致）。
