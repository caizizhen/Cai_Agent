...
> 生成时间：2026-04-23（**发行包 `cai-agent` `0.6.12`**；**S7-01 AC2** 在 **`0.6.11`** 基础上扩展：**`recall_index.build`/`refresh`/`search`**、**`schedule.list`**、**`schedule.add`**、**`gateway.telegram.list`**、**`run.invoke`/`continue.invoke`** → **`CAI_METRICS_JSONL`**（此前 **`memory.health` / `recall.query` / `schedule.stats` / `gateway.status`** 仍有效）；**S8-02/S8-03**（**`0.6.10`** **`perf_ga_gate`/`security_ga_gate`**）；**S8-04**（**`0.6.9`**）；**S7-04**（**`0.6.8`**）；**S7-02/S7-03**（**`0.6.6`–`0.6.7`**）；**S6-04** **`gateway telegram continue-hint`**；**§二 24–26** CLI **MVP**（**`0.6.1`**）；Hermes **34 Story ✅ 34/34**；T1 **`pytest`** 例 **399 passed**（**3 subtests**）+ **`smoke_new_features`**；T2 回归日志 [`docs/qa/runs/regression-20260423-091003.md`](qa/runs/regression-20260423-091003.md) **PASS**；**`QA_SKIP_LOG=1`** **`run_regression.py`** **本日复跑 PASS**（**不写** `docs/qa/runs/` 新文件））  
...
**完成度快照（与 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) §三之二 · 3.0 对齐）**：仅计 ✅ 为 **34/34 = 100%**（**>20%** 同步基线之上；与 §三之二 **3.0** Hermes 行一致）。**S7-01 AC2**（各模块 metrics 全量打点）仍为 **增量**，见 backlog **S7-01 交付注**。
...
| **S7-01**   | 统一指标模型 **`metrics_schema_v1`** | **`cai_agent.metrics`**、**`_maybe_metrics_cli`**、**`CAI_METRICS_JSONL`**：**`observe.summary`/`report`/`export`**；**`memory.health`**、**`recall.query`**、**`schedule.stats`**、**`gateway.status`**（**`0.6.11`**）；**`recall_index.build`/`refresh`/`search`**、**`schedule.list`**、**`schedule.add`**、**`gateway.telegram.list`**、**`run.invoke`/`continue.invoke`**（**`0.6.12`**）；**`docs/schema/METRICS_JSON.zh-CN.md`**；**`test_metrics_jsonl.py`**。**AC2** 其余子命令仍为 **增量**，见 [`HERMES_PARITY_BACKLOG.zh-CN.md`](HERMES_PARITY_BACKLOG.zh-CN.md) **S7-01 交付注** |
...
**QA 等待信号**：**OBS-METRICS**（**`observe`/`memory health`/`recall`/`schedule stats`/`gateway status`/`recall-index` build·refresh·search/`schedule list`·`add`/`gateway telegram list`/`run`/`continue`** 已打点；**`command`/`agent`/`fix-build`** 等仍扩展）、**OBS-RPT**、**OBS-CROSS**、**OBS-EXP**（**`test_observe_export_cli`** + **`smoke_new_features`** 已抽样 **export**）。
