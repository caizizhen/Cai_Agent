...
|------|----------|------|------|----------------|
| T1 | `pytest cai-agent/tests` | 自动化 | **完成** | 例：主线 **432 passed**（**3 subtests passed**）；**`smoke_new_features.py`** → **`NEW_FEATURE_CHECKS_OK`**；**`python -m cai_agent --version`** → **`cai-agent 0.6.17`**（**2026-04-23** 本机复跑；以执行机为准） |
...
| **自动化测试 T1** | `pytest cai-agent/tests` 全绿即视为本条里程碑达成（用例数随版本增加） | **432 passed**（**3 subtests passed**）+ 冒烟 **`NEW_FEATURE_CHECKS_OK`** + **`--version`=`0.6.17`**（**2026-04-23** 本机；见上表 T1） |
...
|------|----------|------|------------|----------|---------------------|
| **QA-1** | **T1** `pytest cai-agent/tests` | 自动化 | CI / 测试 | 每版合并后必跑；失败则阻塞发布 | 主线最近一次：**432 passed**（**3 subtests**）+ **`smoke_new_features.py`** **`NEW_FEATURE_CHECKS_OK`** + **`python -m cai_agent --version`**（**`cai-agent 0.6.17`**）（**2026-04-23**、Windows 本机；以执行机为准） |
