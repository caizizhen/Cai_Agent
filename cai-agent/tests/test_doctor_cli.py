...
class DoctorCliTests(unittest.TestCase):
    ...
    def test_doctor_json_schema(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            mi = pl.get("memory_inject")
            self.assertIsInstance(mi, dict)
            self.assertIn("enabled", mi)
            self.assertIn("max_entries", mi)
            self.assertIn("max_chars", mi)
            self.assertIn("include_stale", mi)
            ...
            pig = pl.get("ecc_pack_ingest_gate")
            self.assertIsInstance(pig, dict)
            self.assertEqual(pig.get("schema_version"), "ecc_pack_ingest_gate_v1")
            self.assertIn("allow", pig)
