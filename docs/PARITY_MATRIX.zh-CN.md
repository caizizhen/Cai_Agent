...
| 任务看板 / 富任务 UI | claude-code | `Done` / `Next` | 已落地 `board --json`（`board_v1`）内嵌 `observe` 与 `observe_schema_version`，支持 `--failed-only` / `--task-id` 过滤，并新增 `failed_summary.recent` 最近失败摘要；更完整 UI 运营面仍在后续迭代 |
