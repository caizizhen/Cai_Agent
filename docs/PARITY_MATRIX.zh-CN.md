...
| 记忆 / 本能 | ECC | `Done` / `Next` | 已落地：`entries.jsonl` 写入前 **v1 schema** 校验；`memory import-entries --dry-run` 结构化校验；`memory import-entries --error-report` 坏数据隔离报告导出（`memory_entries_import_errors_v1`）与人类可读失败摘要；`memory nudge` schema 升级到 `1.1`（`threshold_policy` / `risk_score` / `trend`）+ `nudge-report` 趋势汇总。TTL/自动化提炼仍待办 |
