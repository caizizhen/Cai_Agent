...
`scripts/run_regression.py` shells out to the installed `cai-agent` and runs `scripts/smoke_new_features.py` (JSON envelope checks for `plan` / `run` / `stats` / `sessions` / `observe` / `commands` / `agents` / `cost budget`, plus `schedule add|list|rm`, `memory list --json`, and `memory export --json` in a temporary workspace cwd). `mcp-check` may exit `2` when MCP is disabled; the script treats that as OK. If no inference server is reachable, `models` may fail unless you set `REGRESSION_STRICT_MODELS=1` to require a successful `models` call (for environments where the gateway is always up).
