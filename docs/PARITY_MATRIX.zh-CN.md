...
| WebFetch（HTTPS 只读 GET） | claude-code | `Done` | 内置 `fetch_url` + `[fetch_url]` / `[permissions]`；**`max_redirects`**（**1–50**，默认 **20**）/ **`CAI_FETCH_URL_MAX_REDIRECTS`**；**解析后 IP 校验**（反 DNS rebinding）+ **`allow_private_resolved_ips`** 逃逸；见 [MCP_WEB_RECIPE.zh-CN.md](MCP_WEB_RECIPE.zh-CN.md) 作 MCP 并行方案 |
