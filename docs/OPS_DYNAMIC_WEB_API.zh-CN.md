...
> **状态**：契约与分阶段范围已定稿；**仓库内尚无**随 `cai-agent` 进程发布的只读 HTTP 服务实现。当前运营聚合的**唯一稳定入口**仍为 CLI：**`cai-agent ops dashboard`**（与 **`build_ops_dashboard_payload`** / **`ops_dashboard_v1`** 对齐）。

## 1. 现状（已实现）

| 能力 | 入口 | 说明 |
|------|------|------|
| JSON 载荷 | `ops dashboard --format json`（或 `--json`） | 与 **`cai_agent.ops_dashboard.build_ops_dashboard_payload`** 返回值一致，**`schema_version`=`ops_dashboard_v1`** |
...
- 载荷内 **`cost_aggregate`** 使用 **`aggregate_sessions`**；**`cost_session_limit`** 当前仅在 Python API 中可调（默认 **200**），CLI 未暴露——未来 HTTP 层可列为查询参数。
...
**Query**：与 §3.1 相同（服务端先组 JSON 再 **`build_ops_dashboard_html`**）。
...
| **Phase B** | **侧车**只读 HTTP 服务（如独立 **`uvicorn`** 进程）实现 §3，读磁盘与 CLI 相同数据源 | 部署与 **`cai-agent` 版本**绑定；鉴权 §3.3 |
| **Phase C** | 产品化：SSE/轮询增量、RBAC、多 workspace 路由、与 CI 看板联动 | 单独立项 |

**结论**：**§26（PRODUCT_PLAN）运营面板 MVP** 已在 CLI + 静态 HTML 完成；**「动态」**在 Phase A 即可满足「定时刷新」诉求；Phase B 起才需要本文件 §3 的 HTTP 面。

## 6. 相关索引

- Schema 索引：**[`docs/schema/README.zh-CN.md`](schema/README.zh-CN.md)** § **`ops dashboard`**
- 实现：**`cai_agent/ops_dashboard.py`**（**`build_ops_dashboard_payload`** / **`build_ops_dashboard_html`**）
- 产品表：**[`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md)** §二项 **26** 与 **§三之二 · 3.2「26 后续」**
