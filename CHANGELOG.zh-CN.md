...
- **本地多后端与网关一键配置**：新增 `cai-agent init --preset starter`，从内置 `cai-agent.starter.toml` 生成含 LM Studio / Ollama / vLLM / OpenRouter / 自建 OpenAI 兼容网关等多条 `[[models.profile]]` 的配置；`cai-agent models add --preset` 增加 **`vllm`**、**`gateway`**（`profiles.PRESETS` 约定默认端口与 `VLLM_API_KEY`、`OPENAI_API_KEY`）。README / README.zh-CN、`docs/ONBOARDING.zh-CN.md` 已补充说明；`doctor` 提示 `init --preset starter` 与新 preset。
