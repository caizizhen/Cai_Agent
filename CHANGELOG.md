...
- **`scripts/smoke_new_features.py`**: Also exercises **`cai-agent init --json`** (**`init_cli_v1`**), a **second `init --json`** in the same temp dir (**`config_exists`**, exit **`2`**), **`schedule add|list|rm --json`**, and **`memory`** JSON paths (see changelog **`init --json`** / memory export bullets).
