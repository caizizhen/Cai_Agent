...
**指标（S7-01）**：若设置 **`CAI_METRICS_JSONL`**，成功执行后追加 **`observe.summary`** 等事件；**`memory.*`（含 `health`/`state`/`nudge`/`nudge_report`）**、**`recall`/`recall_index.*`/`schedule.*`/`gateway.*`** 与 **`run`/`continue`/`command`/`agent`/`fix-build` 的 `*.invoke`** 等同见 [METRICS_JSON.zh-CN.md](METRICS_JSON.zh-CN.md) **触发路径**。
