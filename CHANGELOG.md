...
- **`scripts/smoke_new_features.py`**: Also exercises **`cai-agent schedule add|list|rm --json`** inside a temporary workspace directory, asserting **`schedule_add_v1`**, **`schedule_list_v1`** (non-empty **`jobs`**), and **`schedule_rm_v1`** (**`removed`**); plus **`memory list --json`** (empty + seeded **`memory_list_v1`**), **`memory search --json`** (**`memory_search_v1`**), **`memory export-entries --json`** (**`memory_entries_export_result_v1`**), and **`memory export --json`** (**`memory_instincts_export_v1`**).
