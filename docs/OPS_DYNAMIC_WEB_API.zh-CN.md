...
| 交互契约（HM-N04-D01） | `ops serve` + interactions endpoint | **`GET /v1/ops/dashboard/interactions`** 仅 `mode=preview|audit`；**`POST /v1/ops/dashboard/interactions`** 才允许 `mode=apply`（也兼容 preview/audit），所有 apply 写动作都经 `ops_dashboard_action_audit_v1` 留痕 |
| RBAC 写路径（OPS-RBAC-N01） | `ops serve --role viewer\|operator\|admin` | `viewer` 只能读/preview/audit；`operator` 可执行调度重排与 gateway binding 写入；`admin` 额外允许 profile 切换写入；拒绝写动作也会写审计 |
| 多 workspace 发现（OPS-MW-N01） | `GET /v1/ops/workspaces` | 只枚举服务端 `--allow-workspace` allowlist；`include_summary=1` 时为每个 workspace 聚合 dashboard summary |
...
## HM-N04-D01 补充说明（2026-04-28）

- `GET /v1/ops/dashboard/interactions` 若传 `mode=apply` 将返回 `403` + `execute_forbidden`。
- 写动作请使用 `POST /v1/ops/dashboard/interactions`，请求体 JSON 至少包含 `workspace`、`action`，`mode` 默认为 `apply`。
- 该收口确保 apply 通道显式（避免 GET 语义歧义）且与 `ops_dashboard_action_audit_v1` 审计链路一致。
...
## OPS-RBAC-N01 补充说明（2026-04-29）

- `cai-agent ops serve` 新增 **`--role viewer|operator|admin`**，表示该 HTTP 侧车允许的服务端最大角色，默认 `admin` 保持兼容。
- 请求可带 **`X-CAI-Actor`** 与 **`X-CAI-Role`**；请求角色会被服务端最大角色截断，非法角色返回 `invalid_role`。
- `POST /v1/ops/dashboard/interactions` 的 `mode=apply` 按动作校验：`schedule_reorder_preview` 与 `gateway_bind_edit_preview` 需要 `operator`，`profile_switch_preview` 需要 `admin`。
- RBAC 拒绝返回 `403` + `rbac_forbidden`，同时追加 `ops_dashboard_action_audit_v1`，审计行包含 `actor`、`role` 与 `workspace_scope`。

## OPS-MW-N01 补充说明（2026-04-29）

- `GET /v1/ops/workspaces` 返回 `ops_workspaces_v1`，无需 `workspace` query，只基于服务端 `--allow-workspace` 列出可访问根目录。
- 每个 `workspaces[]` 行包含 `workspace`、`exists`、`allowed` 与 dashboard / html / interactions 路由 URL，避免客户端自行拼路径。
- 可选 `include_summary=1` 时为每个存在的 workspace 追加 `ops_dashboard_v1.summary`；支持 `observe_pattern`、`observe_limit`、`schedule_days`、`cost_session_limit` 参数。
