...
| `graph context compaction` | **`context_compaction_summary_v1`**：运行时内部 `context_summary_v1` user message；`summary_source=heuristic` 时包含 `source_message_count`、`role_counts`、`tool_calls_count`、`tool_errors_count`、`tool_calls[]`、`important_paths[]`、`conversation_notes[]`；`summary_source=llm` 时包含 LLM 返回的目标/决策/事实/文件/证据/待办/风险等字段；progress 事件 `phase="compact"` 输出压缩前后 message 数与估算 token 数，LLM 失败降级时另有 `phase="compact_fallback"` | 由 `[context].compact_mode` + `[context].compact_trigger_ratio` 自动触发，或 TUI `/compress` 手动触发 |
| `sessions --compact-eval --json` | **`sessions_compact_eval_v1`**：汇总近期会话的压缩质量；每个会话嵌 **`context_compaction_eval_v1`**，含 `passed`、`score`、`checks`、`message_counts`、`estimated_tokens.reduction_ratio`、路径/工具/marker 保留率；任一失败 exit `2` | `--compact-keep-tail`、`--compact-summary-max-chars`、`--compact-required-marker`、`--compact-min-score` |
