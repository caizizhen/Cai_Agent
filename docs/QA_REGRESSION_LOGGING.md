...
The script also runs **`scripts/smoke_new_features.py`**, which invokes **`python -m cai_agent`** with **`cai-agent/src`** on **`PYTHONPATH`** (same pattern as the main regression steps), for JSON envelopes: `plan` / `run` / `stats` / `sessions` / `observe` / `commands` / `agents` / `cost budget`, plus `init --json`, `schedule add|list|rm|stats --json`, `gateway telegram list --json`, `recall --json`, `memory list --json`, `memory search --json`, `memory export-entries --json`, and `memory export --json` in temp cwds.
