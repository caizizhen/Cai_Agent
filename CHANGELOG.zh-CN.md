...
- **文档**：**[`OPS_DYNAMIC_WEB_API.zh-CN.md`](docs/OPS_DYNAMIC_WEB_API.zh-CN.md)** — 动态运营 Web 的只读 **HTTP API 契约草案**（**`GET /v1/ops/dashboard`** / **`dashboard.html`**，与 **`ops_dashboard_v1`** 对齐）、鉴权草图、**Phase A–C** MVP 分阶段与 **OOS**；**不含**仓库内内置 HTTP 服务。
...
- **文档**：**[`CLOUD_RUNTIME_OOS.zh-CN.md`](docs/CLOUD_RUNTIME_OOS.zh-CN.md)**（Modal/Daytona 等云运行后端 **默认 OOS**、理由与替代路径）；**[`MODEL_ROUTING_RULES.zh-CN.md`](docs/MODEL_ROUTING_RULES.zh-CN.md)**（现有 **`active`/`subagent`/`planner`** 路由说明 + **`[models.routing]`** TOML **草案**，实现仍 backlog）。**`PARITY_MATRIX`** L3 新增云运行 **`OOS`** 行；L2 成本/路由行链至路由草案。
