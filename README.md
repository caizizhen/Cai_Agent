...
| Memory | `cai-agent memory extract` → `memory/entries.jsonl`; `memory list --json`, `memory search`, `memory prune`; instinct paths via `memory instincts`; health nudges via `memory nudge --json` (`--write-file`, `--fail-on-severity`), historical trend via `memory nudge-report --json`, and one-step schedule preset via `schedule add-memory-nudge` |
