...
| 多步工作流 | claude-code / ECC | `Done` | `workflow` 已支持 `parallel_group`、`on_error`、预算、quality gate、步骤级 `when` 条件分支、`retry.max_attempts` 失败恢复与根级 `workflow_aggregate_v1`；任务看板类 UI 见下行 |
| 多模型 profile / TUI 面板 / `/use-model` / session `profile` 字段 | claude-code / ECC | `Done` | `[[models.profile]]` + `cai-agent models`；`models onboarding` 输出 add → capabilities → ping → chat-smoke → use → routing-test 命令链；TUI `Ctrl+M` / `/models` 面板（列表、a/e/d/t）；`/status` 与 `--save-session` 含 `profile`；跨 provider 切换提示 `/compact`；见 [MODEL_SWITCHER_DEVPLAN.zh-CN.md](MODEL_SWITCHER_DEVPLAN.zh-CN.md) §4 |
...
| Browser automation | claude-code / MCP ecosystem | `MCP` / `Done` / `Next` | 默认走 Playwright MCP：见 [BROWSER_MCP.zh-CN.md](BROWSER_MCP.zh-CN.md)；Cai 原生面提供 `tools browser-check --json`、`browser check/task --json` 契约，且 `browser task --execute --confirm --json` 已把 `browser_task_v1.steps[]` 映射为显式确认的 `mcp_call_tool`（`browser_mcp_execution_v1`）。执行尝试会写 `.cai/browser/audit.jsonl`（`browser_audit_event_v1`）并刷新 artifact manifest（`browser_artifact_manifest_v1`）。治理边界见 [BROWSER_PROVIDER_RFC.zh-CN.md](BROWSER_PROVIDER_RFC.zh-CN.md)；更细粒度动作 allowlist 仍为后续 |
...
| 语音 / Bridge / 企业门控特性 | claude-code / Hermes | `OOS` | 依赖官方封闭能力或单独商务；**Voice 产品默认不交付**：评估见 **`docs/rfc/HM_07A_VOICE_BOUNDARY.zh-CN.md`**（**`HM-07a`**），推荐 **MCP** 接入 STT/TTS |
...
| 跨会话 recall | claude-code / Hermes | `Done` / `Next` | `recall` / `recall-index`；JSON `schema_version=1.3`，`--sort recent|density|combined`；0 命中时 `no_hit_reason`；**`recall --evaluate --json`** → **`recall_evaluation_v1`**（审计命中率与负样本 Top）。`recall-index doctor [--fix]`（`recall_index_doctor_v1`，exit 0/2）。性能基准：`scripts/perf_recall_bench.py`（Markdown → `docs/qa/runs/`） |
...
| 成本 / token 策略 | claude-code-analysis | `Done` | `cost budget --check`；`compact + cost budget` 联动；`models suggest`；声明式 **`[models.routing]`**（**`goal_*`** + **`cost_budget_remaining_tokens_below`** 与 **`get_usage_counters`/`[cost].budget_max_tokens`**）；**`models routing-test`** 输出 **`routing_explain_v1`**、base/effective capabilities 与 **`model_fallback_candidates_v1`**（explain-only，`auto_switch=false`）；[MODEL_ROUTING_RULES.zh-CN.md](MODEL_ROUTING_RULES.zh-CN.md) / [MODEL_ROUTING_RULES.md](MODEL_ROUTING_RULES.md) |
...
| 记忆 / 本能 | ECC | `Done` | 已落地：单行 **v1 schema** 校验 + **整文件洁净性门禁**（`append_memory_entry` / `import_memory_entries_bundle` 追加前；`memory extract` 写条目前预检；脏文件拒绝写入，救急 `CAI_MEMORY_ALLOW_DIRTY_ENTRIES_JSONL=1`）；`memory validate-entries`（`memory_entries_file_validate_v1`）；`memory extract --structured` LLM/启发式双模结构化抽取；`memory import-entries`（dry-run + error-report）；`memory prune`（TTL/置信度/overflow/non-active）；`memory nudge` schema `1.1`；`memory health`（评分/grade/freshness/conflict/coverage）；`memory user-model`（行为偏好抽取：工具频率、错误率、goal 摘要）；`memory provider --json/list/use/test`（`memory_provider_contract_v1` / `memory_provider_registry_v1` / `memory_provider_use_v1` / `memory_provider_test_v1`，含 builtin registry 与 `honcho_external` mock HTTP adapter）；`memory state` 状态机；**`auto_extract_skill_after_task`** 可选传入 **Settings** 走 **LLM** 生成草稿（`draft_method`=`llm`\|`template`） |
...
| 插件兼容矩阵（机读） | ECC / 多 harness | `Done` | **`plugin_compat_matrix_v1`**：`plugins --json --with-compat-matrix`；**`doctor --json`** 内 **`plugins`** 捆绑；**`scripts/gen_plugin_compat_snapshot.py --check`** 校验 **`docs/schema/plugin_compat_matrix_v1.snapshot.json`**（内嵌 `plugin_compat_matrix_check_v1`）；说明 [PLUGIN_COMPAT_MATRIX.zh-CN.md](PLUGIN_COMPAT_MATRIX.zh-CN.md) / [PLUGIN_COMPAT_MATRIX.md](PLUGIN_COMPAT_MATRIX.md)；Schema `cai-agent/src/cai_agent/schemas/plugin_compat_matrix_v1.schema.json` |
| 可视化运营面板 | ECC | `Done` / `Next` | **已对齐**：CLI **`ops dashboard`**（JSON/text/html，**`--html-refresh-seconds`**）；**`ops serve`** HTTP（**`/v1/ops/dashboard`** / **`dashboard.html`** / **`dashboard/events`** / **`workspaces`**，**`CAI_OPS_API_TOKEN`** 可选）；**`/v1/ops/dashboard/interactions`** 收口为 **GET preview/audit**、**POST apply/preview/audit**；**`--role viewer|operator|admin`** 与 `X-CAI-Actor` / `X-CAI-Role` 为写路径提供 RBAC，审计行含 `role` / `workspace_scope`；**`ops_workspaces_v1`** 提供 allowlist 多 workspace 发现与 summary 聚合。**仍为 Next**：更完整 operator 路由等 |
| 最小只读 HTTP API（外部驱动 / CI） | Hermes | `Done` / `Next` | **已对齐 v0 + OpenAPI**：**`cai-agent api serve`**（**`CAI_API_PORT`** / **`CAI_API_TOKEN`**）；**`GET /healthz`**、**`/v1/status`**（**`api_status_v1`**）、**`/v1/doctor/summary`**、**`/v1/models`**（**`api_openai_models_v1`**）、**`/v1/models/summary`**、**`/v1/models/capabilities`**、非流式与 SSE **`POST /v1/chat/completions`**（**`api_openai_chat_completion_v1`** / **`api_openai_chat_completion_chunk_v1`**，复用 **`model_response_v1`**）、**`GET /v1/gateway/channel-monitor`**（**`gateway_channel_monitor_v1`**）、**`GET /v1/gateway/slash-catalog`**（**`gateway_slash_catalog_v1`**）、**`POST /v1/tasks/run-due`**（仅 dry_run）、**`GET /openapi.json`** 与 **`api openapi --json`**（**`api_openapi_v1`**）。OpenAPI 已暴露 ops interaction RBAC headers。**仍为 Next**：多 workspace 路由、更完整 operator 网关 |
