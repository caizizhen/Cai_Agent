...
- **`scripts/smoke_new_features.py`**：在仓库根校验 **`plugins --json`**（**`plugins_surface_v1`**，**`--config`** 指向根目录 **`cai-agent.toml`**）；在隔离目录写入最小 **`hooks/hooks.json`** 校验 **`hooks list --json`**（**`hooks_catalog_v1`**）；在空临时工作区校验 **`memory health --json`**（S2-01 **根 `schema_version`=`1.0`**）；并校验 **`init --json`**（**`init_cli_v1`**）、**同一目录二次 `init --json`**（**`config_exists`**、**exit `2`**）、**`schedule add|list|rm --json`**（**`schedule_*_v1`**）、**`schedule stats --json`**（**`schedule_stats_v1`**）、**`gateway telegram list --json`**（**`gateway_telegram_map_v1`**）、**`recall --json`**（**`schema_version` 1.3**、**`no_hit_reason`**）、**`memory list|search|export|export-entries --json`** 等契约；入口为 **`python -m cai_agent`** 并设置 **`PYTHONPATH=cai-agent/src`**（与 **`run_regression.py`** 一致，不依赖 PATH 上的 **`cai-agent`**）。
