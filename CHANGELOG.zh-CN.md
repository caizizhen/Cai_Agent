...
- **`scripts/smoke_new_features.py`**：在**隔离临时工作目录**下校验 **`init --json`**（**`init_cli_v1`**）、**同一目录二次 `init --json`**（**`config_exists`**、**exit `2`**）、**`schedule add|list|rm --json`**（**`schedule_*_v1`**）、**`schedule stats --json`**（**`schedule_stats_v1`**）、**`gateway telegram list --json`**（**`gateway_telegram_map_v1`**）、**`recall --json`**（**`schema_version` 1.3**、**`no_hit_reason`**）、**`memory list|search|export|export-entries --json`** 等契约；入口为 **`python -m cai_agent`** 并设置 **`PYTHONPATH=cai-agent/src`**（与 **`run_regression.py`** 一致，不依赖 PATH 上的 **`cai-agent`**）。
