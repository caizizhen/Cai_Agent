...
| **Unrestricted filesystem (SAFETY-N07 / N08)** | **`unrestricted_mode`**: absolute **`path`/`root`/`cwd`** may leave **`[agent].workspace`** (outside targets still require dangerous confirmation by default); **`dangerous_critical_write_skip_if_unchanged`** keeps critical-write noop semantics; backlog **`docs/SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md`** | **`sandbox.py`** (`resolve_tool_path`), **`tools.py`**, **`graph.py`** (**`merge_tool_call_args`**, optional **`settings_supplier`** so TUI **`/unrestricted`** applies without restart), **`doctor.py`** (avoid Unicode arrows on GBK consoles), **`test_unrestricted_filesystem_paths.py`**, **`test_unrestricted_danger_dispatch_extended.py`**, **`test_graph_tool_payload.py`**, **`test_graph_live_settings.py`**, **`test_tool_glob_search.py`** |
...
| **Structured memory injection (TUI / CLI)** | On by default: **`[memory.inject]`** appends **active** rows from **`memory/entries.jsonl`** to the **system prompt** (same validation as **`memory list`**); **`doctor` / `api_doctor_summary_v1`** include **`memory_inject`**; override with **`CAI_MEMORY_INJECT_*`** env vars | `cai_agent/context.py`, `memory.py`, `config.py`, `doctor.py`, `test_memory_prompt_inject.py`, `README*.md` |
