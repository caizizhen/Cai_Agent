...
The script also runs **`scripts/smoke_new_features.py`** (JSON envelopes for `plan` / `run` / `stats` / `sessions` / `observe` / `commands` / `agents` / `cost budget`, plus `init --json`, `schedule add|list|rm`, `memory list --json`, `memory search --json`, `memory export-entries --json`, and `memory export --json` in temp cwds).
