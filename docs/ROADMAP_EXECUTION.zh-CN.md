...
| `ECC-N02-D05` | `Ready` | `ECC-N02` | pack apply 与 ingest sanitizer 合流 | **`ecc pack-import` / `ecc pack-repair`** 的 **`--apply`** 写入前执行与 **`ecc_ingest_sanitizer_policy_v1`** 一致的只读门禁；阻断时拒绝 apply 并回传机读原因；dry-run/preview 行为不变 | `ECC-N02-D04` `ECC-N04-D02` | pytest + smoke |
