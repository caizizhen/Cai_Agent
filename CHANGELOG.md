...
- **Docs**: **`docs/OPS_DYNAMIC_WEB_API.zh-CN.md`** — draft read-only HTTP contract (**`GET /v1/ops/dashboard`** / **`dashboard.html`**) aligned with **`ops_dashboard_v1`**, auth sketch, Phase A–C MVP for dynamic ops web (no in-repo HTTP server).
...
- **Docs**: **`docs/CLOUD_RUNTIME_OOS.zh-CN.md`** (Modal/Daytona cloud sandboxes **OOS** by default; alternatives); **`docs/MODEL_ROUTING_RULES.zh-CN.md`** (current **`chat_completion_by_role`** routing + **`[models.routing]`** TOML sketch, implementation backlog). **PARITY_MATRIX** L3 cloud row = **`OOS`**; L2 cost row links routing draft.
