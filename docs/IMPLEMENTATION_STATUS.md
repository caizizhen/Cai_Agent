...
| **Unrestricted critical-write noop (SAFETY-N07-D01 / D02)** | With **`[safety].dangerous_critical_write_skip_if_unchanged`**, **`write_file`** targets that hit critical basenames skip basename-level dangerous confirmation when an on-disk UTF-8 file exists (≤512KiB) and either **normalized text matches** or **`.toml`/`.json` parses match after recursive sorted-key canonicalization**; still gated by **`dangerous_confirmation_required`**; backlog: **`docs/SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md`** | `tools.py`, `config.py`, `test_unrestricted_danger_dispatch_extended.py` |
