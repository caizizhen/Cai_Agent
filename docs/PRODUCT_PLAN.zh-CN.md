...
| 18 | **S1-02** `docs/schema/` 各命令 JSON schema 文档 | **部分完成** | 契约汇总于 [`docs/schema/README.zh-CN.md`](schema/README.zh-CN.md)（含 observe … **`mcp-check` / `sessions` / `stats` / `run` 族 / `export`**、**`quality-gate` / `security-scan`**、**`models ping` → `models_ping_v1`**、hooks / doctor / plan / memory / recall）；调度审计与 stats 仍为 [`SCHEDULE_*`](schema/SCHEDULE_AUDIT_JSONL.zh-CN.md) 独立长文 |
...
*文档版本：2026-04-22（§二 S1-02：`models ping --json` 增加 **`schema_version`=`models_ping_v1`**；此前已含 `mcp-check` 与 `run`/`sessions`/`stats`/`export`/`init` 契约摘要；T1 以本机 `pytest cai-agent/tests` 为准。）*
