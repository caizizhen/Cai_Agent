...
**量化完成度（百分比）**：见 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) **§三之二 · 3.0**（当前：**§二 加权约 100%**（与 **`PRODUCT_PLAN` §三之二 · 3.0** 同口径）；**Hermes 34 Story 100%**（**34÷34** ✅；**S7-01 AC2** 已扩 **`CAI_METRICS_JSONL`** 至 **`memory`/`quality_gate`/`security_scan`/`gateway.telegram.resolve_update`/`schedule.add_memory_nudge`**、**`0.6.15`** **`hooks.run_event`/`gateway.telegram.serve_webhook`/`mcp.check`**、**`0.6.16`** **`sessions`/`stats`/`insights`/`plan`/`smoke` 常见面** 及 **`0.6.17`** **`init`/`models`/`workflow`/`release-ga`/`ui`**、**`0.6.18`** **`memory user-model`/`skills hub suggest`** 等））；**`CHANGELOG` §0.7.0** 另记 **run 事件信封**、**TUI `/tasks`**、**`hooks` `script`**、**`memory validate-entries`/`extract --structured`**、**`export --ecc-diff`**、**`skills hub install`** 等（**`pyproject.toml` 版本号** 以仓库实际为准，可能与变更日志小节号不同步））。T1 以本机 **`pytest cai-agent/tests`** 为准，例 **505 passed**（**3 subtests passed**））。**发行包**：**`cai-agent`** 版本以 **`pyproject.toml` / `cai_agent.__version__`** 为准（文档日 **2026-04-23** 全绿基线 **505 passed**）。**冒烟**：`scripts/smoke_new_features.py` 已覆盖 **`gateway telegram list`**、**`gateway status --json`**、**`gateway telegram continue-hint --json`**、**`gateway platforms`/`ops dashboard`/`skills hub manifest`/`memory user-model`/`skills hub suggest`**、**`workflow --json`**（**`task_id`** + **`summary.on_error` / `budget_*`**）、**`run --json` `task_id`**、**`mcp-check`/`security-scan`**、**`observe report --format json`**、**`observe export --format json`**、**`insights --json --cross-domain`** 等；**`insights`** 空窗口 **快速路径** 见 **`__main__._build_insights_payload`**。入口为 **`python -m cai_agent`** + **`PYTHONPATH`**（与 **`run_regression.py`** 一致）。**T2**：可按 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) **§三之三 · QA-2** 使用 **`QA_SKIP_LOG=1`** 复跑 **`run_regression.py`**，**不写**新的 **`docs/qa/runs/*.md`**；**本日** **`QA_SKIP_LOG=1`** **T2** **PASS**（与 **T1** **505 passed**（**3 subtests**）、**`NEW_FEATURE_CHECKS_OK`** 同日复核）。
...
- **`0.6.5`（Hermes S6-04）**：**`gateway telegram continue-hint`**（**`gateway_telegram_continue_hint_v1`**）；**`sprint6-gateway-telegram-testplan.md`** **GTW-CONT**
