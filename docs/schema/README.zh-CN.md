...
- **主要字段（摘要）**：`ok`、`error`、`errors[]`、`provider`、`preset`、`dry_run`、`url`、`goal`、`session`、`artifacts`、`steps[]`、`preflight`、`execution`。默认仍为 dry-run，只输出计划与 **`browser_mcp_execution_v1`** 计划调用。
- **BRW-N04 执行器**：`browser task --execute --confirm --json` 才会把 `steps[]` 映射为显式 **`mcp_call_tool`** 调用；未带 `--confirm` 时返回 `execution.error=explicit_confirmation_required` 且 `calls[].status=refused`。当前映射：`navigate` → `browser_navigate`，`delegate_to_browser_mcp` → `browser_snapshot`（先取可审计页面快照，后续动作需新计划/确认）。`execution.calls[]` 含 `permission_key`、`tool`、`args`、`status` 与 `result_preview`。
- **BRW-N05 审计与产物**：非 dry-run 的拒绝/确认路径会追加 **`.cai/browser/audit.jsonl`**，每行为 **`browser_audit_event_v1`**（`event=browser.execution`、`ok`、`confirmed`、`call_status_counts`、`artifact_manifest`）。同时刷新 **`.cai/browser/artifacts-manifest.json`**（**`browser_artifact_manifest_v1`**），枚举 `screenshots/`、`downloads/`、`traces/` 下的文件（`kind`、`path`、`relative_path`、`size_bytes`、`mtime_iso`），并汇总 `calls_count` / `call_status_counts`。`execution.audit` 返回 **`browser_audit_summary_v1`**，`execution.artifact_manifest` 内嵌 manifest 摘要。
...
- **`gateway channel-monitor`（GW-CHAN-N01）**：stdout **`gateway_channel_monitor_v1`**；复用 `gateway_production_summary_v1.platforms[*].channel_monitoring`，输出 `platforms[]`（`id`、`production_state`、`channels_count`、`error_count_total`、`has_latency_data`、`channels[]`），顶层含 `platform_filter`、`only_errors`、`platforms_count`、`channels_count`、`error_count_total`。CLI 支持 **`--platform telegram|discord|slack|teams|signal|email|matrix`** 与 **`--only-errors`**；API **`GET /v1/gateway/channel-monitor`** 同源输出，query 为 `platform` / `only_errors`。
- **`gateway slash-catalog`（GW-SLASH-N01）**：stdout **`gateway_slash_catalog_v1`**；离线列出 Discord Application Commands、Slack `/cai` 子命令与 Teams command list，字段含 `platforms[]`（`id`、`surface`、`registration`、`commands[]`）与顶层 `commands_count` / `summary.execute_capable_count`。API **`GET /v1/gateway/slash-catalog`** 同源输出。
...
- **多 workspace 发现（OPS-MW-N01）**：**`GET /v1/ops/workspaces`** 返回 **`ops_workspaces_v1`**，只枚举服务端 `--allow-workspace` allowlist 内的根目录；字段含 `workspaces_count`、`include_summary` 与 `workspaces[]`（`workspace`、`exists`、`allowed`、`dashboard_url`、`dashboard_html_url`、`interactions_url`）。可选 query **`include_summary=1`** 时为每个存在的 workspace 内嵌同源 `ops_dashboard_v1.summary`，并复用 `observe_pattern`、`observe_limit`、`schedule_days`、`cost_session_limit` 参数。
...
- **RBAC（OPS-RBAC-N01）**：`cai-agent ops serve --role viewer|operator|admin` 设置服务端最大角色；请求可带 **`X-CAI-Actor`** 与 **`X-CAI-Role`**（请求角色不会超过服务端最大角色）。`viewer` 只能读/preview/audit，`operator` 可执行调度重排与 gateway binding 写入，`admin` 额外允许 `profile_switch_preview` 的 `apply`。RBAC 拒绝返回 `error=rbac_forbidden`，并写入 **`ops_dashboard_action_audit_v1`**；交互响应含 **`rbac`**（**`ops_dashboard_rbac_v1`**：`actor`、`role`、`workspace_scope`）。审计行新增 `role` 与 `workspace_scope` 字段。
...
- **`schema_version`**：`workflow_run_v1`；根级 **`task_id`** 与 **`task.task_id`** 同源（与 `run --json` 对齐）；另有 **`subagent_io_schema_version`**：**`1.1`** 与 `subagent_io`（`inputs` / `merge` / `outputs`；每步 **`agent_template_id`** 与可选 **`rpc_step_input`/`rpc_step_output`**）、根级可选 **`agent_templates`**、`steps`、`summary`、`events`、`task`、**`quality_gate`** 摘要与可选 **`post_gate`**（执行后置门禁时为 **`quality_gate_result_v1`**）。
...
- **WF-N01**：步骤可选 **`when`**（如 `{ "step": "repair", "status": "ok|failed|skipped" }`）用于条件分支；条件不满足时该步骤输出 **`skipped: true`**、**`skip_reason`** 为 `when_condition_false` / `when_dependency_missing` / `when_false`。步骤可选 **`retry.max_attempts`**（或 `max_attempts` / `max_retries`，上限 5），失败后产生 **`workflow.step.retrying`** 事件，最终步骤含 **`attempts`** / **`max_attempts`**。根级 **`aggregate: true`** 输出 **`workflow_aggregate_v1`**，按成功步骤生成 `answers_by_name`，并列出失败与跳过步骤。
- **Workflow 后置质量门禁**：根级可选 **`quality_gate`**：`true` 或对象。`true` 表示按当前 **`[quality_gate]`** 配置在 workflow 成功后执行一次后置门禁；对象可覆盖 **`compile`** / **`test`** / **`lint`** / **`typecheck`** / **`security_scan`** / **`report_dir`**（相对路径相对 **workflow 文件目录**）。输出含 **`quality_gate`**（`requested` / `ran` / `ok` / `failed_count` / `skip_reason` / `report_dir`），门禁实际执行时根级 **`post_gate`** 为 **`quality_gate_result_v1`**；事件流追加 **`workflow.quality_gate.completed`** 或 **`workflow.quality_gate.skipped`**；`workflow.finished` / `summary` 同步包含 **`quality_gate_*`** 字段。若 gate 失败，`task.error` 为 **`workflow_quality_gate_failed`**。
...
| `memory provider` | **`memory_provider_contract_v1`**：列出默认 **`local_entries_jsonl`** 与 **`local_user_model_sqlite`** provider、路径、存在性、读/写 surface、计数，以及 **`user_model_provider_coverage`**；含 **`builtin_registry`**（`memory_provider_builtin_registry_v1`）与 `external_adapters[]` | 默认只读；空工作区不会初始化 SQLite store |
| `memory provider list/use/test` | **`memory_provider_registry_v1`** / **`memory_provider_use_v1`** / **`memory_provider_test_v1`**：`list` 暴露 active provider 与 registry；`use --id` 写 `.cai/memory-provider.json`；`test --id honcho_external` 在显式 `CAI_MEMORY_EXTERNAL_MOCK_URL` 下探测 `/health`，返回远端 `schema_version` 与健康状态 | `honcho_external` 是 mock HTTP contract adapter；默认不启用网络或真实凭据 |
...
| `graph context compaction` | **`context_compaction_summary_v1`**（schema: `cai-agent/src/cai_agent/schemas/context_compaction_summary_v1.schema.json`）：运行时内部 `context_summary_v1` user message；`summary_source=heuristic` 时包含 `source_message_count`、`role_counts`、`tool_calls_count`、`tool_errors_count`、`tool_calls[]`、`important_paths[]`、`conversation_notes[]`；`tool_calls[]` 可含 `tool_kind`（`test`/`traceback`/`git_diff`/`search`/`read`/`command`/`generic`）、`paths[]`、`evidence[]`、`failure_summary[]`、`diff_stats`；多代压缩合并已有 summary 时可含 `merged_summary_count`、`merged_source_message_count`；`summary_source=llm` 时包含 LLM 返回的目标/决策/事实/文件/证据/待办/风险等字段；progress 事件 `phase="compact"` 输出压缩前后 message 数与估算 token 数，LLM 失败、非 JSON、不变小或 retention gate 失败时另有 `phase="compact_fallback"`；retention gate 输出 **`context_compaction_retention_v1`**（schema: `context_compaction_retention_v1.schema.json`），失败 `error` 形如 `llm_compaction_quality_failed: paths_retained,tools_retained` | 由 `[context].compact_mode` + `[context].compact_trigger_ratio` 自动触发，或 TUI `/compress` 手动触发 |
| `sessions --compact-eval --json` | **`sessions_compact_eval_v1`**：汇总近期会话的压缩质量；每个会话嵌 **`context_compaction_eval_v1`**（schema: `cai-agent/src/cai_agent/schemas/context_compaction_eval_v1.schema.json`），含 `passed`、`score`、`checks`、`message_counts`、`estimated_tokens.reduction_ratio`、路径/工具/marker 保留率；任一失败 exit `2` | `--compact-keep-tail`、`--compact-summary-max-chars`、`--compact-required-marker`、`--compact-min-score` |
...
| `GET /v1/gateway/channel-monitor` | **`gateway_channel_monitor_v1`**；与 CLI `gateway channel-monitor --json` 同源，支持 `platform` / `only_errors` query |
| `GET /v1/gateway/slash-catalog` | **`gateway_slash_catalog_v1`**；与 CLI `gateway slash-catalog --json` 同源，离线公开 slash/command list 能力目录 |
