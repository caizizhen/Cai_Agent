...
| **解限文件系统（SAFETY-N07 / N08）** | **`unrestricted_mode`**：**绝对路径可访问 `[agent].workspace` 外**（文件工具与 **`run_command cwd`**；工作区外默认仍须二次确认）；**`dangerous_critical_write_skip_if_unchanged`**（关键 basename 文本/TOML·JSON noop）；清单 **`docs/SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md`** | **`sandbox.py`**、**`tools.py`**、**`graph.py`**（**`merge_tool_call_args`**、**`settings_supplier`** 热加载 TUI 解限）、**`doctor.py`**（Windows GBK 控制台避免箭头符号崩溃）、**`test_unrestricted_filesystem_paths.py`**、**`test_unrestricted_danger_dispatch_extended.py`**、**`test_graph_tool_payload.py`**、**`test_graph_live_settings.py`**、**`test_tool_glob_search.py`** |
...
| **结构化记忆注入（TUI / CLI）** | 默认开启：**`[memory.inject]`** 将 **`memory/entries.jsonl`** 中 **active** 条目注入 **system prompt**（与 **`memory list`** 同源校验）；**`doctor` / `api_doctor_summary_v1`** 含 **`memory_inject`**；**`CAI_MEMORY_INJECT_*`** 环境变量可覆盖 | **`context.py`**、**`memory.py`**、**`config.py`**、**`doctor.py`**、**`test_memory_prompt_inject.py`**、**`README*.md`** |
