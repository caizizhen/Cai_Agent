...
**量化完成度（百分比）**：见 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) **§三之二 · 3.0**（当前：**§二 加权约 100%**；**Hermes 34 Story 100%**（**34÷34** ✅））。**`CHANGELOG` §0.7.0** 记 **run 事件信封**、**TUI `/tasks`**、**`hooks` `script`**、**`memory validate-entries`/`extract --structured`**、**`ops serve`** / **`ops dashboard --html-refresh-seconds`**、**`memory user-model export`**、**`export --ecc-diff`**、**`skills hub install`** 等（**`pyproject.toml` 版本号** 以仓库实际为准）。T1 以本机 **`pytest cai-agent/tests`** 为准，**564 passed**（**3 subtests passed**；**2026-04-24** 全量回归）量级。**发行包**：**`cai-agent`** 版本以 **`pyproject.toml` / `cai_agent.__version__`** 为准。**冒烟**：`scripts/smoke_new_features.py` 已覆盖 **`gateway telegram list`**、**`gateway status --json`**、**`gateway telegram continue-hint --json`**、**`gateway platforms`/`ops dashboard`/`memory user-model --json`/`memory user-model export`**、**`skills hub manifest`/`skills hub suggest`**、**`workflow --json`**、**`run --json` `task_id`**、**`mcp-check`/`security-scan`**、**`observe report`/`observe export`**、**`insights --json --cross-domain`** 等。入口为 **`python -m cai_agent`** + **`PYTHONPATH`**（与 **`run_regression.py`** 一致）。**T2**：可按 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) **§三之二 · 3.3（QA-2）** 使用 **`QA_SKIP_LOG=1`** 复跑 **`run_regression.py`**，**不写**新的 **`docs/qa/runs/*.md`**。
...
- **运营面板 Phase A/B**：**`ops dashboard --format html --html-refresh-seconds`**；**`cai-agent ops serve`**（**`/v1/ops/dashboard`** / **`dashboard.html`**）；单测 **`test_ops_http_server.py`**、**`test_ops_dashboard_html.py`**（含刷新 CLI）
- **用户模型导出（RFC 切片）**：**`memory user-model export`** → **`user_model_bundle_v1`**；单测 **`test_memory_user_model_export.py`**；**`smoke_new_features`** 已校验 export
