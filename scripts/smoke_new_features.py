...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="cai-smoke-memhealth-") as mh_td:
        ...
        pex = _run(
            [*cli, "export", "--config", str(mh_cfg), "-w", mh_td, "--target", "cursor"],
            cwd=mh_td,
        )
        if pex.returncode != 0:
            errs.append(f"export cursor for pack-repair exit {pex.returncode} stderr={pex.stderr!r}")
        ppr = _run(
            [
                *cli,
                "ecc",
        ...
        if ppr.returncode != 0:
            errs.append(f"ecc pack-repair exit {ppr.returncode} stderr={ppr.stderr!r}")
        else:
        ...
        if phd.returncode != 0:
            errs.append(f"ecc home-diff exit {phd.returncode} stderr={phd.stderr!r}")
        else:
            hdd = json.loads((phd.stdout or "").strip())
            if hdd.get("schema_version") != "ecc_structured_home_diff_bundle_v1":
                errs.append(f"ecc home-diff schema {hdd.get('schema_version')!r}")
