...
| **ECC ingest draft tranche (ECC-N04-D01 / D02)** | Added ingest draft snapshots: **`ecc_asset_registry_v1.snapshot.json`** (source/license/signature/version/trust) and **`ecc_ingest_sanitizer_policy_v1.snapshot.json`** (`deny_exec`, dangerous hook isolation, review decision flow), plus bilingual sanitizer policy docs | `docs/schema/ecc_asset_registry_v1.snapshot.json`, `docs/ECC_04B_INGEST_SANITIZER_POLICY.zh-CN.md`, `docs/schema/ecc_ingest_sanitizer_policy_v1.snapshot.json` |
...
- **`pytest cai-agent/tests`** (repo root: **`python -m pytest -q cai-agent/tests`**): **803 passed**, **3 subtests passed**; **`PYTHONPATH=cai-agent\src`**.
