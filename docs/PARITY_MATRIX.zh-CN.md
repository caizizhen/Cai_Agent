...
| 成本 / token 策略 | claude-code-analysis | `Done` / `Next` | `cost budget --check`；`compact + cost budget` 联动；`models suggest`（启发式任务-profile 推荐）；声明式 **`[models.routing]`** 规则表见 [MODEL_ROUTING_RULES.zh-CN.md](MODEL_ROUTING_RULES.zh-CN.md)（**草案**，实现仍 **Next**） |
