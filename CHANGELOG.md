...
- **Recall sort modes (Hermes S3-01)**: `cai-agent recall --json` and `recall-index search|benchmark` now accept `--sort recent|density|combined` (default `recent`). Payload `schema_version` bumps to **`1.2`** and includes `sort` plus an explicit `ranking` description; keyword-density scoring uses full matched message bodies (not only snippets) for more stable density ordering.
