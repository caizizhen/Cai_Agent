...
| Browser automation | claude-code / MCP ecosystem | `MCP` / `Next` | 默认走 Playwright MCP：见 [BROWSER_MCP.zh-CN.md](BROWSER_MCP.zh-CN.md)；Cai 原生面仅提供 `tools browser-check --json`、`browser check/task --json` 契约；治理边界见 [BROWSER_PROVIDER_RFC.zh-CN.md](BROWSER_PROVIDER_RFC.zh-CN.md) |
...
| 记忆 / 本能 | ECC | `Done` | 已落地：单行 **v1 schema** 校验 + **整文件洁净性门禁**（`append_memory_entry` / `import_memory_entries_bundle` 追加前；`memory extract` 写条目前预检；脏文件拒绝写入，救急 `CAI_MEMORY_ALLOW_DIRTY_ENTRIES_JSONL=1`）；`memory validate-entries`（`memory_entries_file_validate_v1`）；`memory extract --structured` LLM/启发式双模结构化抽取；`memory import-entries`（dry-run + error-report）；`memory prune`（TTL/置信度/overflow/non-active）；`memory nudge` schema `1.1`；`memory health`（评分/grade/freshness/conflict/coverage）；`memory user-model`（行为偏好抽取：工具频率、错误率、goal 摘要）；`memory provider --json`（`memory_provider_contract_v1`，local entries / user-model SQLite / future external adapter 边界）；`memory state` 状态机；**`auto_extract_skill_after_task`** 可选传入 **Settings** 走 **LLM** 生成草稿（`draft_method`=`llm`\|`template`） |
...
| 可视化运营面板 | ECC | `Done` / `Next` | **已对齐**：CLI **`ops dashboard`**（JSON/text/html，**`--html-refresh-seconds`**）；**`ops serve`** 只读 HTTP（**`/v1/ops/dashboard`** / **`dashboard.html`** / **`dashboard/events`**，**`CAI_OPS_API_TOKEN`** 可选）；**`/v1/ops/dashboard/interactions`** 收口为 **GET preview/audit**、**POST apply/preview/audit**，保持 `ops_dashboard_interactions_v1` + `ops_dashboard_action_audit_v1` 审计链路。**仍为 Next**：RBAC、多租户路由等 |
| 最小只读 HTTP API（外部驱动 / CI） | Hermes | `Done` / `Next` | **已对齐 v0 + OpenAPI**：**`cai-agent api serve`**（**`CAI_API_PORT`** / **`CAI_API_TOKEN`**）；**`GET /healthz`**、**`/v1/status`**（**`api_status_v1`**）、**`/v1/doctor/summary`**、**`/v1/models`**（**`api_openai_models_v1`**）、**`/v1/models/summary`**、**`/v1/models/capabilities`**、非流式与 SSE **`POST /v1/chat/completions`**（**`api_openai_chat_completion_v1`** / **`api_openai_chat_completion_chunk_v1`**，复用 **`model_response_v1`**）、**`POST /v1/tasks/run-due`**（仅 dry_run）、**`GET /openapi.json`** 与 **`api openapi --json`**（**`api_openapi_v1`**）。**仍为 Next**：RBAC、多 workspace 路由、更完整 operator 网关 |
...
| Docker runtime backend | Hermes | `Done` / `Next` | **已对齐**：`[runtime] backend="docker"` 支持既有 **`container`** / `docker exec` 模式与新增 **`image`** / `docker run --rm` 模式；支持 **`workdir`**、**`volume_mounts`**、**`cpus`**、**`memory`**，并在 **`doctor_runtime_v1.describe`** 暴露 mode/image/workdir/volumes/limits。**仍为 Next**：真实 CI 镜像矩阵。 |
| SSH runtime backend | Hermes | `Done` / `Next` | **已对齐**：`[runtime] backend="ssh"` 支持 key、`known_hosts`、严格 host key、连接超时诊断；**`doctor_runtime_v1.describe`** 暴露 `ssh_binary_present`、key/known_hosts 存在性与 audit 配置；可选 **`runtime_ssh_audit_v1`** JSONL 审计默认不记录命令明文。**仍为 Next**：真实远端 smoke 与多主机编排。 |
