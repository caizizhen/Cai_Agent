...
```
  ...
  `run_regression.py` 会调用已安装的 `cai-agent`，并执行 `scripts/smoke_new_features.py`（校验 `plan` / `run` / `stats` / `sessions` / `observe` / `commands` / `agents` / `cost budget` 及**临时工作区**下的 `schedule add|list|rm`、`memory list --json`、`memory export --json` 等 JSON 契约）。`mcp-check` 在 MCP 未启用时退出码可能为 `2`，脚本已按预期处理。若本地无推理服务，`models` 可能失败，可设置环境变量 `REGRESSION_STRICT_MODELS=1` 强制要求 `models` 成功（用于网关已就绪的环境）。
