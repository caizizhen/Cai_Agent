...
> 生成时间：2026-04-23（**发行包 `cai-agent` `0.6.13`**；**S7-01 AC2** 已扩 **`CAI_METRICS_JSONL`**：含 **`0.6.11`–`0.6.12`** 既有路径，及 **`0.6.13`** **`command`/`agent`/`fix-build`·`invoke`**、**`memory.state`/`nudge`/`nudge_report`**、**`recall_index.benchmark`/`info`/`clear`/`doctor`**、**`schedule.rm`/`run_due`/`daemon`**、**`gateway.telegram`** **`bind`/`get`/`unbind`/`continue_hint`/`allow_*`**；**S8-02/S8-03**（**`0.6.10`** **`perf_ga_gate`/`security_ga_gate`**）；**S8-04**（**`0.6.9`**）；**S7-04**（**`0.6.8`**）；**S7-02/S7-03**（**`0.6.6`–`0.6.7`**）；**S6-04** **`gateway telegram continue-hint`**；**§二 24–26** CLI **MVP**（**`0.6.1`**）；Hermes **34 Story ✅ 34/34**；T1 **`pytest`** 例 **406 passed**（**3 subtests**）+ **`smoke_new_features`**；T2 回归日志 [`docs/qa/runs/regression-20260423-091003.md`](qa/runs/regression-20260423-091003.md) **PASS**；**`QA_SKIP_LOG=1`** **`run_regression.py`** **本日复跑 PASS**（**不写** `docs/qa/runs/` 新文件））  
