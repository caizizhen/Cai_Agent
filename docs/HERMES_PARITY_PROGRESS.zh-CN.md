...
> 生成时间：2026-04-23（**发行包 `cai-agent` `0.6.0`**；进度统计与 **QA 测试移交清单** 同步见 [`PRODUCT_PLAN.zh-CN.md`](PRODUCT_PLAN.zh-CN.md) **§三之二**；T1 **`pytest`+`smoke_new_features`** 本机复跑见 §三之二；T2 回归日志 [`docs/qa/runs/regression-20260423-091003.md`](qa/runs/regression-20260423-091003.md) **PASS**；亦可 **`QA_SKIP_LOG=1`** 跑 `run_regression.py` **不写** `docs/qa/runs/` 新文件）  
