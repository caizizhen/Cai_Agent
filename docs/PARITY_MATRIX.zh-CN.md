...
| 跨会话 recall | claude-code / Hermes | `Done` / `Next` | `recall` / `recall-index`；JSON `schema_version=1.3`，`--sort recent|density|combined`；0 命中时 `no_hit_reason`。`recall-index doctor [--fix]`（`recall_index_doctor_v1`，exit 0/2）。压测脚本仍待办 |
