...
- **Memory nudge trend report**: Added `cai-agent memory nudge-report` to aggregate `memory/nudge-history.jsonl` into an operator-friendly trend snapshot (`history_total`, `severity_counts`, `latest_severity`, `severity_trend`, `avg_recent_sessions`, `avg_memory_entries`). Supports `--history-file`, `--limit`, and `--json`; useful for QA/ops drift monitoring across runs.
