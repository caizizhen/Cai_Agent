...
- **Recall index acceleration**: `cai-agent recall` now supports index-backed search (`--use-index`) and index build/update via `--build-index` with configurable path (`--index-file`, default `.cai/recall-index.jsonl`). The index stores per-session flattened text + mtime and is reused when unchanged, reducing repeated full JSON scans on larger histories. `--refresh-index` forces full rebuild.
