...
| **解限文件系统（SAFETY-N07 / N08）** | **`unrestricted_mode`**：**绝对路径可访问 `[agent].workspace` 外**（文件工具与 **`run_command cwd`**；工作区外默认仍须二次确认）；**`dangerous_critical_write_skip_if_unchanged`**（关键 basename 文本/TOML·JSON noop）；清单 **`docs/SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md`** | **`sandbox.py`**、**`tools.py`**、**`test_unrestricted_filesystem_paths.py`**、**`test_unrestricted_danger_dispatch_extended.py`** |
