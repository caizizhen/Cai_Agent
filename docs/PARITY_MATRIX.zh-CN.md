...
| 最小只读 HTTP API（外部驱动 / CI） | Hermes | `Done` / `Next` | **已对齐 v0**：**`cai-agent api serve`**（**`CAI_API_PORT`** / **`CAI_API_TOKEN`**）；**`GET /healthz`**、**`/v1/status`**（**`api_status_v1`**）、**`/v1/doctor/summary`**、**`/v1/models`**（**`api_openai_models_v1`**）、**`/v1/models/summary`**、**`/v1/models/capabilities`**、非流式与 SSE **`POST /v1/chat/completions`**（**`api_openai_chat_completion_v1`** / **`api_openai_chat_completion_chunk_v1`**，复用 **`model_response_v1`**）、**`POST /v1/tasks/run-due`**（仅 dry_run）；RFC [`HM_02_MINIMAL_SERVER_CONTRACT.zh-CN.md`](rfc/HM_02_MINIMAL_SERVER_CONTRACT.zh-CN.md)。**仍为 Next**：OpenAPI、更多只读路由、与 **`ops serve`** 统一网关 |
