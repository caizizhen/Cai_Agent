...
- **`scripts/smoke_new_features.py`**: Also exercises **`init --json`** (**`init_cli_v1`**), a **second `init --json`** in the same temp dir (**`config_exists`**, exit **`2`**), **`schedule add|list|rm --json`**, **`schedule stats --json`** (**`schedule_stats_v1`**), and **`memory`** JSON paths; it now shells **`python -m cai_agent`** with **`cai-agent/src`** on **`PYTHONPATH`** (same as **`run_regression.py`**, no reliance on a **`cai-agent`** script on **`PATH`**).
