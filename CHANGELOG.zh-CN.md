...
- **Memory Nudge 历史报告**：新增 `cai-agent memory nudge-report`，从 `memory/nudge-history.jsonl` 聚合历史快照并输出趋势统计（`severity_counts`、`severity_trend`、`latest_severity`、`avg_recent_sessions`、`avg_memory_entries`）。支持 `--history-file`、`--limit` 与 `--json`，可用于 QA/运维观察记忆健康变化。
