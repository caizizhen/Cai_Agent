...
- **Memory nudge trend report**: Added `cai-agent memory nudge-report` to aggregate `memory/nudge-history.jsonl` into an operator-friendly trend snapshot (`schema_version=1.1`, `history_total`, `severity_counts`, `latest_severity`, `severity_trend`, `severity_jumps`, `avg_recent_sessions`, `avg_memory_entries`). Supports `--history-file`, `--days` (time window filter), `--limit`, and `--json`; useful for QA/ops drift monitoring across runs. `memory nudge --write-file` appends the same JSON line to the default history file (or `--history-file`) unless it points at the same path as `--write-file`.
