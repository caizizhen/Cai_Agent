...
| **自动化测试 T1** | `pytest cai-agent/tests` 全绿即视为本条里程碑达成（用例数随版本增加） | **351 passed**（**2026-04-23** 本机复核；见上表 T1） |
...
|------|----------|------|------------|----------|---------------------|
| **QA-1** | **T1** `pytest cai-agent/tests` | 自动化 | CI / 测试 | 每版合并后必跑；失败则阻塞发布 | 主线最近一次：**351 passed**（**2026-04-23** Windows 本机 `pytest cai-agent/tests -q` 复核；以执行机为准） |
| **QA-2** | **T2** `python scripts/run_regression.py` | 自动化 | 测试 | 仓库根执行；关注 `docs/qa/runs/regression-*.md` | 脚本已固定 `PYTHONPATH` + `python -m cai_agent`；**`smoke_new_features`** 亦 **`python -m cai_agent`**；开发侧 **2026-04-23** 复核：[`regression-20260423-083409.md`](qa/runs/regression-20260423-083409.md) **PASS** |
...
*文档版本：2026-04-23（§三之二 **3.0**：**§二 加权约 85%**（**22÷26≈84.6%**）；**Hermes 34 Story 约 62%**（**21÷34**）；**开发项 21** **完成**（`run`/`workflow` 根级 **`task_id`**）；**22、23** **部分完成**；**`smoke_new_features`** 已含 **`workflow --json` `task_id`**；S5-01/S5-02 见 **`HERMES_PARITY_PROGRESS.zh-CN.md`**；**QA 证据刷新**：T1 **`pytest` 351 passed**、T2 **[`regression-20260423-083409.md`](qa/runs/regression-20260423-083409.md) PASS**（HEAD `045730d`，Windows）。）*
