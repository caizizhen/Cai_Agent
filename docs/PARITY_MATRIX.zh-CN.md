...
| 可视化运营面板 | ECC | `Done` / `Next` | **已对齐**：CLI **`ops dashboard`**（JSON/text/html，**`--html-refresh-seconds`**）；**`ops serve`** 只读 HTTP（**`/v1/ops/dashboard`** / **`dashboard.html`** / **`dashboard/events`**，**`CAI_OPS_API_TOKEN`** 可选）；**`/v1/ops/dashboard/interactions`** 输出 **`ops_dashboard_interactions_v1`**，当前仅支持 schedule reorder / gateway bind-edit dry-run 预览。**仍为 Next**：真实 apply、RBAC、多租户路由等 |
