...
| 2 | 工作区工具 + 沙箱 + Shell 白名单 | **完成** | `sandbox.py`（**`resolve_tool_path`**）、`tools.py`；解限：**绝对路径可越工作区**（须二次确认）；关键写 noop（`dangerous_critical_write_skip_if_unchanged`）；[`SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md`](SAFETY_UNRESTRICTED_BACKLOG.zh-CN.md) |
