...
- **`scripts/smoke_new_features.py`**：在**隔离临时工作目录**下校验 **`init --json`**（**`init_cli_v1`**）、**同一目录二次 `init --json`**（**`config_exists`**、**exit `2`**）、**`schedule add|list|rm --json`**（**`schedule_*_v1`**）、**`memory list|search|export|export-entries --json`** 等契约。
