...
### S1-03 错误码规范（补齐剩余命令）
- **现状**：`memory nudge`、`recall` 等命令已实现 exit 0/2，但 `memory health`（未实现）、`schedule stats`（未实现）、`observe report`（未实现）等新命令的错误码还没有
...
|----------|------|--------|------|---------|
| **S2-01** | `memory health` 统一评分命令 | P1 | L | [sprint2-memory-health-testplan.md](qa/sprint2-memory-health-testplan.md) |
...
**开发关键文件**：`cai-agent/src/cai_agent/__main__.py`（新增 `memory health` 子解析器与 `_build_memory_health_payload` 函数）  
**QA 等待信号**：S2-01 提测后，QA 运行 `python3 -m pytest -q tests/test_memory_health*.py`
...
| Story ID | 标题 | 优先级 | 估算 | 测试计划 |
|----------|------|--------|------|---------|
| **S3-01** | recall 排序策略升级（`--sort density/combined`） | P1 | L | [sprint3-recall-v2-testplan.md](qa/sprint3-recall-v2-testplan.md) RCL-RANK-001~006 |
...
**开发关键文件**：
- `__main__.py`：`recall` 新增 `--sort` 参数；`recall-index` 新增 `doctor` 子命令
