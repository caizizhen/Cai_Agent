...
- **未开发功能队列（2026-04-25）**：**`DEVELOPER_TODOS`** §0.4 新增未开发功能队列（P0 批次 `HM-02c` / `CC-03c` / `ECC-03b` 已 **Done**；P1 队列含 Teams gateway、docker/SSH runtime、Dashboard 高级交互、gateway 生产化、插件 CI snapshot；OOS 条目附替代方案指针）。同步 **`ROADMAP_EXECUTION`** §10 与 **`GAP_TRACKER.md`**。
- **Explore 批次收口（HM-03c / ECC-03a / HM-06a / HM-07a）**：在 **`docs/rfc/`** 增补评估/设计结论文档：**`HM_03C_NEXT_GATEWAY_PLATFORMS.zh-CN.md`**、**`ECC_03A_PLUGIN_VERSION_GOVERNANCE.zh-CN.md`**、**`HM_06A_RUNTIME_BACKEND_ASSESSMENT.zh-CN.md`**、**`HM_07A_VOICE_BOUNDARY.zh-CN.md`**；**`ROADMAP_EXECUTION`** §10 对应 issue 标为 **Done**（交付物为文档）。**`DEVELOPER_TODOS`** §**0.3.1** 为推荐执行顺序表。
