...
> 目标：一页看清“已实现 / 正在实现 / 未来实现”。

## 1. 已实现（Done）

- `MODEL-P0`：模型接入地基（gateway、capabilities、health/chat-smoke、routing explain）
- `CC-N01`：安装/升级/修复主路径
- `CC-N02`：反馈与诊断链路
...
- `HM-N06`：第二批 gateway 平台（Explore）
- `CC-N05`、`ECC-N05`：桌面/控制面探索（Explore）
- `CC-N07`、`HM-N11`：远程与云 runtime（Conditional）

## 4. OOS / 边界

- `CC-N06` 原生 WebSearch/Notebook：OOS，继续 MCP 优先
