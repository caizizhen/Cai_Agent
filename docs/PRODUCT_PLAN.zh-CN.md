...
| 18 | **S1-02** `docs/schema/` 各命令 JSON schema 文档 | **部分完成** | 契约汇总于 [`docs/schema/README.zh-CN.md`](schema/README.zh-CN.md)（含 observe … **`plugins` → `plugins_surface_v1`**、**`mcp-check` / `sessions`（`sessions_list_v1`）/ `stats` / `run` 族 / `export`（`export_cli_v1`）**、**`quality-gate` / `security-scan`**、**`models ping` → `models_ping_v1`**、**`models fetch` → `models_fetch_v1`**、**`cost budget` → `cost_budget_v1`**、hooks / doctor / plan / memory / recall）；**`commands`/`agents` → `commands_list_v1`/`agents_list_v1`**；调度审计与 stats 仍为 [`SCHEDULE_*`](schema/SCHEDULE_AUDIT_JSONL.zh-CN.md) 独立长文 |
...
*文档版本：2026-04-22（§二 S1-02：`cost budget` 单行 JSON 增加 **`cost_budget_v1`**；`sessions`/`commands`/`agents` 等见 schema README；T1 **334 passed**。）*
