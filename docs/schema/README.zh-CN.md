...
**指标（S7-01）**：若设置 **`CAI_METRICS_JSONL`**，成功执行后追加 **`observe.summary`** 等事件；**`memory.health` / `recall.query` / `schedule.stats` / `gateway.status`**、**`recall_index.*` / `schedule.list` / `schedule.add` / `gateway.telegram.list` / `run.invoke` / `continue.invoke`** 等同见 [METRICS_JSON.zh-CN.md](METRICS_JSON.zh-CN.md) **触发路径**。
