...
The script also runs **`scripts/smoke_new_features.py`** (JSON envelopes for `plan` / `run` / `stats` / `sessions` / `observe` / `commands` / `agents` / `cost budget`, plus `schedule add|list|rm` and `memory list --json` in a temp cwd).
