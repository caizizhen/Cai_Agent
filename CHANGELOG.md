...
- **Init & model presets for local stacks and gateways**: `cai-agent init --preset starter` copies a bundled `cai-agent.starter.toml` with explicit profiles for LM Studio, Ollama, vLLM, OpenRouter, and a placeholder self-hosted OpenAI-compatible gateway. `cai-agent models add --preset` now includes **`vllm`** and **`gateway`** (plus existing presets); `profiles.PRESETS` documents default ports (`8000` / `8080`) and env vars (`VLLM_API_KEY`, `OPENAI_API_KEY`). README / README.zh-CN / `docs/ONBOARDING.zh-CN.md` describe the flow; `doctor` hints at `init --preset starter` and the new CLI presets.
